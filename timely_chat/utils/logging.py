...
def __get_caller_module_name() -> str:
    ...
    return module.__name__ if module else "<anonymous>"
...
def create_logger(
    name: Optional[str] = None, log_output_path: Optional[str] = None, level=logging.INFO, rank: int = 0
) -> Logger:
    """
    logger를 생성합니다.
    ...
    if rank == 0 and log_output_path:
        filename = os.path.join(log_output_path, datetime.now().strftime("%Y-%m-%d_%H-%M-%S_%f.log"))
